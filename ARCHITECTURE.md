...
>    - **【背景特例】沉浸式空间氛围 (Spatial Aura) 与全局切换：** 允许且鼓励在纯黑底色上使用极其微弱、极具质感的**深暗色流光渐变**作为一种“宇宙暗物质/极光”般的背景点缀。为了满足用户个性化，**背景必须是全局可切换的**（通过 BackgroundManager 控制，目前支持 极简纯黑 / 动态流光），且弹窗、设置面板等覆盖层必须保持绝对透明（禁止使用深色蒙版），让底层背景完全透出。
